        ...
        <div className="fixed inset-0 z-50 bg-black/95 flex flex-col animate-in fade-in duration-200">
          ...
          {galleryMode === 'slideshow' ? (
            <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-6 relative overflow-hidden">
              ...
              {/* Thumbnails Filmstrip at the bottom */}
              {images.length > 1 && (
                <div className="mt-4 flex items-center gap-2 max-w-full overflow-x-auto py-2 px-4 no-scrollbar">
                  {images.map((img, i) => (
                    <button
                      key={i}
                      type="button"
                      onClick={() => setSelectedPhotoIndex(i)}
                      className={`relative shrink-0 w-16 h-12 rounded-md overflow-hidden border-2 transition cursor-pointer ${
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {images.map((img, idx) => (
                  <div
                    ...
                    className="relative aspect-[4/3] rounded-xl overflow-hidden group bg-neutral-900 border border-neutral-800 cursor-pointer hover:border-neutral-500 transition"
                  >
                    <img
                      src={img}
                      alt={`Photo ${idx + 1}`}
                      ...
                      referrerPolicy="no-referrer"
                    />
                    <span className="absolute bottom-2 right-2 px-2 py-0.5 rounded bg-black/75 text-white font-mono text-[10px]">
                      #{idx + 1}
                    </span>
                  </div>
                ))}
